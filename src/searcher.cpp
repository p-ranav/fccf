...
namespace search
{
...

  {
	...
	std::exit(-1);
      ...
      std::pair<std::string_view, std::string_view> filename_and_contents = {filename, haystack};
      ...
      if (clang_visitChildren(
			  ...
			  {
			    auto filename_and_contents = (std::pair<std::string_view, std::string_view>*)client_data;
				...
				std::string_view member_function_name = (const char*)clang_getCursorDisplayName(c).data;
				std::string_view query = searcher::m_query.data();
				
				if (member_function_name.find(query) != std::string_view::npos) {
